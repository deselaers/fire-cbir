...
FIRELIBS = $(LIBDIR)/libClustering.a $(LIBDIR)/libRetriever.a $(LIBDIR)/libDistanceFunctions.a $(LIBDIR)/libFeatureExtractors.a $(LIBDIR)/libFeatures.a  $(LIBDIR)/libImage.a $(LIBDIR)/libCore.a  
...
# Distances--------------------------------------------------------
LIBDISTANCES_SOURCES = DistanceFunctions/dist_bm25.cpp DistanceFunctions/dist_globallocalfeaturedistance.cpp DistanceFunctions/dist_idm.cpp DistanceFunctions/dist_lfhungarian.cpp DistanceFunctions/dist_lfsigemd.cpp DistanceFunctions/dist_metafeature.cpp DistanceFunctions/dist_mpeg7.cpp DistanceFunctions/dist_rast.cpp DistanceFunctions/dist_smart2.cpp DistanceFunctions/dist_textfeature.cpp DistanceFunctions/dist_tfidf.cpp DistanceFunctions/emd.cpp DistanceFunctions/dist_weightedl1.cpp	
LIBDISTANCES_OBJECTS := $(patsubst %.o,$(OBJDIR)/%.o,$(LIBDISTANCES_SOURCES:.cpp=.o))
$(LIBDIR)/libDistanceFunctions.a: $(LIBDISTANCES_OBJECTS)

# Retriever -------------------------------------------------------
LIBRETRIEVER_SOURCES = Retriever/database.cpp Retriever/distancemaker.cpp Retriever/featureloader.cpp Retriever/getscoring.cpp Retriever/imagecomparator.cpp  Retriever/largebinaryfeaturefile.cpp Retriever/largefeaturefile.cpp Retriever/maxentscoring.cpp Retriever/maxentscoringfirstandsecondorder.cpp Retriever/maxentscoringsecondorder.cpp Retriever/retriever.cpp Retriever/server.cpp Retriever/querycombiner.cpp
...
# Classifiers -------------------------------------------------------
CLASSIFIERS_SOURCES = Classifiers/createlfkdtree.cpp
CLASSIFIERS_OBJECTS := $(patsubst %.o,$(OBJDIR)/%.o,$(CLASSIFIERS_SOURCES:.cpp=.o))
CLASSIFIERS_PROGRAMS := $(patsubst Classifiers/%.o,$(BINDIR)/%,$(CLASSIFIERS_SOURCES:.cpp=.o))
$(BINDIR)/createlfkdtree: $(OBJDIR)/Classifiers/createlfkdtree.o $(FIRELIBS)
...
CLUSTERING_SOURCES = Clustering/emclustercenter2vectorfeature.cpp Clustering/imageclusterer.cpp Clustering/jfclustering.cpp Clustering/lfclustering.cpp Clustering/trainclustervariancesdiscriminative.cpp Clustering/trainclusterweightsdiscriminative.cpp Clustering/visualizeclusterpositions.cpp
...
$(BINDIR)/trainclustervariancesdiscriminative: $(OBJDIR)/Clustering/trainclustervariancesdiscriminative.o $(FIRELIBS)
$(BINDIR)/trainclusterweightsdiscriminative: $(OBJDIR)/Clustering/trainclusterweightsdiscriminative.o $(FIRELIBS)
$(BINDIR)/visualizeclusterpositions: $(OBJDIR)/Clustering/visualizeclusterpositions.o $(FIRELIBS)

# TextRetriever -------------------------------------------------------
#TEXTIR_SOURCES = TextRetriever/diag.cpp TextRetriever/Endian.cpp TextRetriever/German.cpp TextRetriever/Indexing.cpp TextRetriever/Inversion.cpp TextRetriever/IR_Engine.cpp TextRetriever/net.cpp TextRetriever/ParseCtrlFile.cpp TextRetriever/Pool.cpp TextRetriever/Porter.cpp TextRetriever/QtControl.cpp TextRetriever/QtOutlineTree.cpp TextRetriever/QtOutlineTree.moc.cpp TextRetriever/RecallPrecision.cpp TextRetriever/Retrieval.cpp TextRetriever/Stemmer.cpp TextRetriever/Term.cpp 
#TEXTIR_PROGRAMS := $(BINDIR)/ir_engine
#$(BINDIR)/ir_engine:  $(TEXTIR_SOURCES)
#	$(CXX) -o $@ $^ -lm $(TEXTIR_FLAGS)
...
# Misc ----------------------------------------------------------------
MISC_SOURCES = Misc/collage.cpp Misc/dbpca.cpp Misc/facefeatureprocessor.cpp Misc/featurescomparator.cpp Misc/eigenfacer.cpp Misc/histogramnormalization.cpp Misc/mosaic.cpp Misc/create_classes.cpp Misc/pcavectortoimage.cpp  Misc/testscaleinvariantfeatures.cpp Misc/testsparsehistogramfeature.cpp  Misc/visualizelocalfeatures.cpp 
...
all: $(RETRIEVER_PROGRAMS) $(FEATEX_PROGRAMS) $(CLASSIFIERS_PROGRAMS) $(CLUSTERING_PROGRAMS) $(TEXTIR_PROGRAMS) $(MISC_PROGRAMS) $(TOOLS_PROGRAMS)
