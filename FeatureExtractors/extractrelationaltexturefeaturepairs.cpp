...
void USAGE() {
  cout  << "USAGE:" << endl
        ...
        << "    --suffix <suffix>" << endl
        << "          default: pairs.rel.histo.gz" << endl
        << "    --steps <steps> " << endl
        ...
        << "          default: rel:x1=0:y1=0:x2=4:y2=0" << endl
        << "    --R <noofsteps> " << endl
...
int main(int argc, char** argv) {
  ...
  if(cl.size()<2) USAGE();
  string suffix=cl.follow("rel.histopair.gz","--suffix");
  ...
  string kernelfunction = "rel:x1=0:y1=0:x2=4:y2=0";
  ...
  for(uint i=0;i<infiles.size();++i) {
    ...
    // Histogram berechnen
    HistogramFeature completeHisto = getRelationalFeatureHistogram(img, kernelfunction);
    ...
    for(uint i=0;i<sizes.size();++i) {
      ...
      HistogramFeature partHisto = getRelationalFeatureHistogramPart(img, kernelfunction, startx, stopx, starty, stopy);
